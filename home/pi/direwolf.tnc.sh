...
if [ $NEWRIGNUMBER = DTR ]; then  
  ...
  sed -i "s/\#PTT CM108 DEVICEFILE/PTT CM108 \/dev\/$NEWDEVICEFILE/" /tmp/direwolf.tnc.conf
elif [ $USBPRESENT -eq 0 -o $NEWRIGNUMBER = GPIO ]; then
   sed -i "s/\#PTT GPIOD/PTT GPIOD/" /tmp/direwolf.tnc.conf
else
...
direwolf  -d h -d h -d h -d t -d o -p -q d -t 0 -c /run/direwolf.tnc.conf |& grep --line-buffered -v PTT_METHOD > /home/pi/direwolf.log   &
